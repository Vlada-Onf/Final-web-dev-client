...
var Utils = NewClass(
  ...
  },
  {
    ...
    IsFileSystemSupported: function () {
      ...
      window.requestFileSystem(
        window.PERSISTENT,
        EU_PERSISTENT_FS_SIZE,
        initFunction,
        HandleErrorFileSystem,
      )
    },
    ...
    SetSessionStorageItems: function (name, items, protect) {
      ...
      if (
        !this.SetSessionStorageItem(name + 'Count', length.toString(), protect)
      ) {
        return false
      }
      ...
      for (var i = 0; i < length; i++) {
        if (
          !this.SetSessionStorageItem(name + i.toString(), items[i], protect)
        ) {
          for (var j = 0; j < i; j++) {
    ...
    GetSessionStorageItem: function (name, decode, protect) {
      try {
        ...
        if (protect) {
          return this.euSign.UnprotectDataByPassword(item, null, !decode)
        } else {
          if (!decode) return item
          else return this.euSign.Base64Decode(item)
        }
      } catch (e) {
        return null
      }
    ...
    GetSessionStorageItems: function (name, decode, protect) {
      var length = this.GetSessionStorageItem(name + 'Count', false, false)
    ...
    GetIOSVersion: function () {
      ...
      var verArray = [
        parseInt(verString[1], 10),
        parseInt(verString[2], 10),
        parseInt(verString[3] || 0, 10),
      ]
    ...
    CreateObjectURL: function (file) {
      if (this.IsIOS() && this.GetIOSVersion() < 7) {
    ...
    LoadFile: function (file, onSuccess, onFail) {
      ...
      fileReader.onloadend = fileLoaded(file, onSuccess, onFail)
    ...
    LoadFilesToArray: function (files, onSuccess, onFail) {
      ...
      var fileReader = new FileReader()
      fileReader.onloadend = fileLoaded(files, 0, [], onSuccess, onFail)
    ...
    IsFolderExists: function (folderName) {
      if (window.localStorage.getItem(folderName) != null) return true

      return false
    },
    CreateFolder: function (folderName) {
      if (this.IsFolderExists(folderName)) return true

      var files = JSON.stringify([])
      window.localStorage.setItem(folderName, files)
    },
    DeleteFolder: function (folderName) {
      if (!this.IsFolderExists(folderName)) return false
    ...
    GetFiles: function (folderName) {
      if (!this.IsFolderExists(folderName)) return null
    ...
    GetDataFromServerAsync: function (path, onSuccess, onFail, asByteArray) {
      try {
        ...
        httpRequest.onload = function () {
          ...
          } else onFail(httpRequest.status)
        }
